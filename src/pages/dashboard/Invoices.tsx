...
const Invoices = () => {
  ...
  const fetchInvoices = async () => {
    ...
    if (error) toast.error("Error fetching invoices");
  ...
  const handlePaymentStatusToggle = async (id: string, currentStatus: string) => {
    ...
    if (error) toast.error("Error updating payment status");
    else {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) toast.error("Error deleting invoice");
    else {
...
Your invoice from PAVILO is ready.
...
Please check the attached PDF.
    ...
    setWhatsappNumber("");
    toast.success("PDF generated! Opening WhatsApp...");
  };

  // ==========================================================
  // 📤 Auto-upload + download PDF
  // ==========================================================
  const generatePDF = async (invoiceId: string) => {
    ...
    const pdf = generateEnhancedPDF(invoice, items || [], template || undefined);
    ...
    // Upload to Drive via Supabase Edge Function
    await uploadToDrive(invoiceId, user.id, base64PDF, pdfFileName);

    toast.success("Invoice saved & uploaded to Google Drive");
  };
  ...
  const uploadToDrive = async (invoiceId: string, userId: string, pdfBlobBase64: string, fileName: string) => {
    ...
    if (error) {
      ...
      toast.error("Failed to upload invoice to Google Drive");
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link to="/dashboard/invoices/new">
            <Button><Plus className="mr-2 h-4 w-4" />{t("createInvoice")}</Button>
          </Link>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground">{t("noDataFound")}</TableCell></TableRow>
              ) : (
                  ...
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => handlePaymentStatusToggle(invoice.id, invoice.payment_status)}>
                          {invoice.payment_status === "paid" ? (
                        ...
                        <Button variant="ghost" size="icon" onClick={() => generatePDF(invoice.id)}>
      ...
      <Dialog open={whatsappDialog} onOpenChange={setWhatsappDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t("enterWhatsAppNumber")}</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            />
            <p className="text-xs text-muted-foreground">{t("includeCountryCode")}</p>
          </div>

          <DialogFooter>
            <Button variant="outline" onClick={() => setWhatsappDialog(false)}>{t("cancel")}</Button>
            <Button onClick={sendWhatsAppMessage}>{t("openWhatsApp")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </DashboardLayout>
